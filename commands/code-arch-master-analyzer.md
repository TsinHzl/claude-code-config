---
name: code-arch-master-analyzer
description: 当用户要求分析目录、理解代码结构、走读代码、追踪调用图、生成技术文档或分析架构演进时使用。关键词：保姆级、源码解析、架构分析、走读代码、变更分析、技术深潜、explain codebase、analyze code。
trigger: 当用户输入包含以下任意关键词时触发：保姆级、源码解析、帮我看懂、架构分析、初学者、变更分析、技术深潜、设计模式变化、架构演进、变更报告、代码改动、diff分析、看懂代码、核心原理、走读代码、code walkthrough、explain codebase、analyze code、生成文档、技术文档
---

# Skill: code-arch-master-analyzer（终极源码架构分析专家）
...
| 变更分析 / 改动 / diff / 架构演进 | **Mode B: 变更深度剖析** | `[仓库名]_变更深度剖析.md` |
| 两者都要 / 全量分析 / 完整分析 | **Mode C: 全量分析（A+B）** | 同时生成两份文档 |

若用户未明确指定，默认执行 **Mode A**，并在开头告知用户可追加 Mode B。
...
- **零基础**：通过生活类比 + 大白话，能理解"这个模块是干什么的"
- **中级**：通过 Mermaid + 技术原理，能理解"为什么这么设计"
...
1. **覆盖性检查**：对比 Step 2A 扫描到的所有核心文件，确认每个主要模块都有对应章节
...
```
Read 文档（必须）→ Edit 追加：
- 项目概述（职责 2-3 句 + 核心特性 3-5 条 + 技术栈表格）
- 目录结构（关键文件树 ≤20 行，每行带注释说明职责）
```

### 第三步：追加核心概念
```
Read 文档（必须）→ Edit 追加：
- 2-3 个核心类/接口，每个含完整三层分析
- 单次内容 ≤400 行
```
...
```
Read 文档（必须）→ Edit 追加：
- 1-2 个核心业务流程的 Mermaid 图（sequenceDiagram 或 flowchart）
- 每图配大白话翻译 + 最难理解的点说明
```
...
```
Read 文档（必须）→ Edit 追加：
- 错误如何被捕获、包装、映射到状态码/用户提示
- 边界输入校验策略（是否用 Zod/Joi/自定义 validator）
- 安全相关逻辑（鉴权检查、敏感数据处理）
```
...
**💡 心智模型**：
[最贴切的生活类比，例：就像餐厅出菜速度跟不上下单速度，系统需要决定是"让顾客等"还是"拒绝新单"...]
...
- [ ] 所有核心概念都有三层分析（大白话 / 技术原理 / 实现细节）
- [ ] 调用图追踪完整（Step 2A 扫描到的主要文件都有对应章节）
- [ ] 所有 Mermaid 图语法正确闭合，特殊字符已加引号，节点 ≤12 个
- [ ] 所有代码片段 ≤15 行，每行有中文注释，含文件路径:行号
- [ ] 难点部分覆盖了代码中**所有**难以理解的地方（不能跳过任何一个）
- [ ] 每个难点有完整实现追踪（调用方 → 中间层 → 底层，不断链）
- [ ] 包含错误处理与安全章节（哪怕内容简短也必须有）
